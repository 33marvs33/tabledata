FIREMANDATA = JSON.parse(localStorage.getItem("fireman")) || [];
...
if(FIREMANDATA.length <= 0) {
    alert('no datas')
}else if(FIREMANDATA) {
    FIREMANDATA.forEach(data => {
        displayData(data)
    })
}
})
...
formSubmit.addEventListener("submit", (e) => {
  ...
  FIREMANDATA.push(data);
  localStorage.setItem("fireman", JSON.stringify(FIREMANDATA));
  displayData();
});
...
findBtn.addEventListener('click', () => {
    ...
    const find = FIREMANDATA.filter(key => key.firstname == findValue)
    findInput.value = ""
    displayDatas2(find)
})

selector.addEventListener('change', (e) => {
    const completed = FIREMANDATA.filter(key => key.paid == true);
    const notcompleted = FIREMANDATA.filter(key => key.paid == false);
...
function displayData() {
  tableDivs.innerHTML = "";
  FIREMANDATA.forEach((data, index) => {
    tableDivs.innerHTML += `
        <table>
        <tr id="table_row">
...
function deletes() {
    ...
    deleteBtn.forEach(deleteBtn => {
        deleteBtn.addEventListener('click', (e) => {
            ...
            // asssignnig the filtered data to FIREMANDATA then put it to localStorage//
            FIREMANDATA = FIREMANDATA.filter(data => data.id !== targetId);
             localStorage.setItem('fireman', JSON.stringify(FIREMANDATA))
        })
