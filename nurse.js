NURSEDATA = JSON.parse(localStorage.getItem("nurse")) || [];
...
findBtn.addEventListener('click', () => {
    ...
    const find = NURSEDATA.filter(key => key.firstname == findValue)
    findInput.value = ""
    displayDatas2(find)
})

selector.addEventListener('change', (e) => {
    const completed = NURSEDATA.filter(key => key.paid == true);
    const notcompleted = NURSEDATA.filter(key => key.paid == false);
...
function deletes() {
    ...
    deleteBtn.forEach(deleteBtn => {
        deleteBtn.addEventListener('click', (e) => {
            ...
            // asssignnig the filtered data to NURSEDATA then put it to localStorage//
            NURSEDATA = NURSEDATA.filter(data => data.id !== targetId);
             localStorage.setItem('nurse', JSON.stringify(NURSEDATA))
        })
