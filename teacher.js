...
findBtn.addEventListener('click', () => {
    ...
    const find = TEACHERDATA.filter(key => key.firstname == findValue)
    findInput.value = ""
    displayDatas2(find)
})

selector.addEventListener('change', (e) => {
    const completed = TEACHERDATA.filter(key => key.paid == true);
    const notcompleted = TEACHERDATA.filter(key => key.paid == false);
...
function deletes() {
    ...
    deleteBtn.forEach(deleteBtn => {
        deleteBtn.addEventListener('click', (e) => {
            ...
            // asssignnig the filtered data to TEACHERDATA then put it to localStorage//
            TEACHERDATA = TEACHERDATA.filter(data => data.id !== targetId);
             localStorage.setItem('teacher', JSON.stringify(TEACHERDATA))
        })
