OTHERDATA = JSON.parse(localStorage.getItem("other")) || [];
...
findBtn.addEventListener('click', () => {
    ...
    const find = OTHERDATA.filter(key => key.firstname == findValue)
    findInput.value = ""
    displayDatas2(find)
})

selector.addEventListener('change', (e) => {
    const completed = OTHERDATA.filter(key => key.paid == true);
    const notcompleted = OTHERDATA.filter(key => key.paid == false);
...
function deletes() {
    ...
    deleteBtn.forEach(deleteBtn => {
        deleteBtn.addEventListener('click', (e) => {
            ...
            // asssignnig the filtered data to OTHERDATA then put it to localStorage//
            OTHERDATA = OTHERDATA.filter(data => data.id !== targetId);
             localStorage.setItem('other', JSON.stringify(OTHERDATA))
        })
